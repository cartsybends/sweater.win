...
VERSION = "61 · Goalie Mode & Sharper Map"
...
function drawMapCities() {
  ...
  // National capitals are readable from a continent view; state and provincial
  // ones join a little later. Screen-width caps keep phones calm.
  const capitalOpacity = national => {
    const start = Math.min(national ? 120 : 110, cw * (national ? .3 : .28));
    const full = Math.min(national ? 90 : 75, cw * (national ? .22 : .19));
